...
function isHeadless() {
    return false; // Naukri Akamai WAF blocks headless mode. Must run headed.
}
...
async function trySessionLogin(page) {
    ...
    // Check if Akamai WAF blocked us
    const title = await page.title();
    if (/access denied/i.test(title)) {
        throw new Error(
            `Naukri blocked this browser (Akamai WAF). You cannot use headless: true. ` +
            `Please revert to headed mode (headless: false) to bypass the firewall.`
        );
    }
